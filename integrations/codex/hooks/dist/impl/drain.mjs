...
function pruneStale(cfg = {}) {
  try {
    ...
    for (const r of dirEntries(join2(root, "runs"))) {
      ...
      expire(join2(rd, "files.json"), 7 * DAY);
      expire(join2(rd, "drain.lock"), 60 * SEC);
      expire(join2(rd, "ledger.jsonl"), 30 * DAY);
...
function appendLedger(dir, runId, row) {
  try {
    if (!isObject3(row) || !dir || !safeSegment(runId)) return false;
    const p = ledgerPath(dir, runId);
    if (!ensureDir(dirname5(p))) return false;
...
import { appendFileSync, mkdirSync as mkdirSync2, renameSync as renameSync3, statSync as statSync5 } from "node:fs";
...
function log(cfg, level, msg, fields = {}) {
  try {
    ...
    const dir = join8(resolveDataDir(cfg), "logs");
    try {
      mkdirSync2(dir, { recursive: true });
    } catch {
      return;
    }
    const file = join8(dir, FILE2);
...
function rotateIfNeeded(dir, file) {
  try {
    if (statSync5(file).size < MAX_BYTES) return;
    renameSync3(file, join8(dir, PREV));
  } catch {
  }
}
...
async function refreshPins(cfg, runId) {
  try {
    if (isObject5(cfg) && cfg.pins === false) return { ok: true, refreshed: false, pins: 0 };
    if (!cachePath2(cfg, runId)) return { ok: false, refreshed: false, pins: 0, error: "unusable run id" };
    const current = rawCache(cfg, runId);
    if (current && Math.abs(Date.now() - num3(current.at, 0)) < PIN_TTL_MS) {
...
function cachePath2(cfg, runId) {
  ...
  return join10(runDir(isObject5(cfg) ? cfg : {}, runId), CACHE_FILE3);
}
function rawCache(cfg, runId) {
  const p = cachePath2(cfg, runId);
  if (!p) return null;
  const raw = readJson(p, null);
  return isObject5(raw) && raw.v === CACHE_VERSION3 ? raw : null;
}
...
function endpointOf2(cfg) {
  return trimSlash(isObject5(cfg) ? cfg.endpoint : "");
}
...
import { existsSync as existsSync6, readdirSync as readdirSync3, readFileSync as readFileSync6, statSync as statSync6 } from "node:fs";
import { basename as basename2, dirname as dirname6, join as join11, resolve as resolve4 } from "node:path";
...
var drain_exports = {};
import { renameSync as renameSync5, unlinkSync as unlinkSync4, writeFileSync as writeFileSync4 } from "node:fs";
...
function quarantine(cfg, runId, batch, res) {
  ...
  for (const entry of batch) {
    ...
    if (!from) continue;
    const dir = join13(dirname7(from), "rejected");
    ensureDir(dir);
    const to = join13(dir, basename3(from));
    try {
      renameSync5(from, to);
    } catch {
      try {
        writeFileSync4(to, JSON.stringify(entry.item ?? null), "utf8");
        unlinkSync4(from);
      } catch {
      }
    }
