...
function ledgerPath(dir, runId) {
  return join9(runDir({ dataDir: dir }, safeSegment(runId) || "unknown"), LEDGER_FILE);
}
function turnLedgerRow(turn, runId, now = Date.now()) {
  const t = isObject5(turn) ? turn : {};
  const r = isObject5(t.recall) ? t.recall : {};
  const ev = isObject5(t.used_evidence) ? t.used_evidence : null;
  ...
  return {
    ...
    prompt_id: str4(t.prompt_id),
    session_id: str4(t.session_id),
    ...
    recalled: Array.isArray(t.recalled) ? t.recalled.filter((v) => typeof v === "string" && v.trim()).map((v) => v.trim()) : [],
    recall: {
      rung: num(r.rung),
      ...
      empty_reason: str4(r.empty_reason)
    },
...
function appendLedger(dir, runId, row) {
  try {
    if (!isObject5(row) || !dir || !safeSegment(runId)) return false;
    const p = ledgerPath(dir, runId);
    if (!ensureDir(dirname7(p))) return false;
    const line = `${JSON.stringify(row)}
`;
    const fd = openSync3(p, "a+");
    let size = 0;
    try {
      ...
      if (st.size > 0) {
        ...
        readSync2(fd, last, 0, 1, st.size - 1);
        if (last[0] !== 10) prefix = "\n";
      }
...
var LEDGER_VERSION, LEDGER_FILE, LEDGER_MAX_BYTES, LEDGER_KEEP_BYTES, LEDGER_ROW_TTL_MS, LEDGER_PREVIEW_BYTES, LEDGER_REDACTION;
...
function fileChanges(toolName, toolInput, toolResponse) {
  const out = [];
  try {
    const input = isObject6(toolInput) ? toolInput : {};
...
function responseKind(response) {
  if (!isObject6(response) || typeof response.type !== "string") return "";
...
function recordFileChanges(cfg, runId, changes) {
  try {
    ...
    const ok = writeJsonAtomic(join10(dir, FILES_FILE), {
      version: VERSION,
      updated_at: now,
      files
    });
...
function readStored(cfg, runId) {
  try {
    if (!runId) return [];
    const stored = readJson(join10(runDir(cfg, runId), FILES_FILE), null);
    ...
    for (const f of stored.files) {
      ...
      out.push({
        path,
        kinds,
        occurrences: Math.max(1, Math.trunc(numOr2(f.occurrences, 1))),
        at: Math.max(0, Math.trunc(numOr2(f.at, 0)))
      });
