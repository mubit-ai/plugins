...
function carryPath(cfg, runId) {
  ...
  return join7(runDir(cfg, runId), "carry.json");
}
...
function seenPath(cfg, runId, sessionId) {
  ...
  if (!session) return "";
  return join12(runDir(cfg, runId), SEEN_DIR, `${session}.json`);
}
...
// ../claude-code/hooks/src/recall-refresh.mjs
var recall_refresh_exports = {};
...
var REFRESH_BUDGET_MS, HARNESS_BUDGET_MS, MIN_PROMPT_CHARS, MAX_QUERY_CHARS, MAX_ID, SUPPRESS;
var init_recall_refresh = __esm({
  async "../claude-code/hooks/src/recall-refresh.mjs"() {
    init_config();
    init_hook();
    init_log();
    init_rank();
    ...
    init_seen();
    init_state();
    REFRESH_BUDGET_MS = 1e4;
    HARNESS_BUDGET_MS = REFRESH_BUDGET_MS + 2e3;
    MIN_PROMPT_CHARS = 8;
    MAX_QUERY_CHARS = 2e3;
    MAX_ID = 128;
...
await init_recall_refresh().then(() => recall_refresh_exports);
