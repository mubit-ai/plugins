...
function loadConfig(env = process.env) {
  ...
  const cachePath2 = join6(dataDir2, CACHE_FILE);
...
function cachePath(cfg, runId) {
  if (!safeSegment(runId)) return "";
  return join10(runDir(isObject4(cfg) ? cfg : {}, runId), CACHE_FILE2);
}
function oneLine2(v) {
  try {
    if (typeof v !== "string") {
...
var CACHE_VERSION2, CACHE_FILE2, PIN_TTL_MS, MAX_PINS, MAX_PIN_CHARS, MAX_PIN_TOKENS, HEADING, EMPTY;
var init_pins = __esm({
  "../claude-code/lib/pins.mjs"() {
    init_assemble();
    init_state();
...
function resumePath(cfg, runId) {
  if (!safeSegment(runId)) return "";
  return join11(runDir(cfg, runId), "resume.json");
}
...
function recordRules(cfg, runId, entries) {
  try {
    ...
    const ok = writeJsonAtomic(join12(dir, RULES_FILE), {
      version: VERSION,
      updated_at: Date.now(),
      rules: merged
    });
...
function readRules(cfg, runId) {
  try {
    if (!runId) return [];
    const stored = readJson(join12(runDir(cfg, runId), RULES_FILE), null);
...
function policyPath(cfg) {
  ...
  return join13(resolveDataDir(cfg), "policy", `${hash}.json`);
}
...
var RUNG2_MIN_BUDGET_MS, ENTRY_TYPES, QUERY_LIMIT, CROSS_RUN_MIN_BUDGET_MS, RANK_MODES, CONTEXT_LIMIT, RESUME_SECTIONS, RESUME_ENTRY_TYPES, POLICY_TTL_MS, ENDPOINT_HASH_LEN;
...
import { basename as basename2, dirname as dirname5, join as join14, resolve as resolve4 } from "node:path";
...
function sessionPath(file) {
  return join14(dataDir({}), "sessions", `${file}.json`);
}
...
function hasGitDir2(start) {
  try {
    ...
    for (let i = 0; i < 24; i++) {
      if (existsSync6(join14(cur, ".git"))) return true;
...
function seenPath(cfg, runId, sessionId) {
  ...
  if (!session) return "";
  return join15(runDir(cfg, runId), SEEN_DIR, `${session}.json`);
}
...
var SEEN_TTL_MS, MAX_SEEN_REFS, SEEN_DIR, MAX_SESSION_SEGMENT;
var init_seen = __esm({
  "../claude-code/lib/seen.mjs"() {
    init_runid();
    init_state();
