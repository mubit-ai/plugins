...
var ACTIVITY_ROUTES, EXPORT_TIMEOUT_MS, EXPORT_OPTS, EXPORT_FORMAT, COMPACT_KEYS, COMPACT_CONTENT_CHARS, PAGE_MIN, PAGE_MAX, PAGE_DEFAULT, SCAN_MAX_ENTRIES, SCAN_MAX_PAGES, SCAN_BUDGET_MS, DERIVED_KEYS;
...
var activity_src_exports = {};
__export(activity_src_exports, {
...
import { realpathSync, existsSync as existsSync6, mkdirSync as mkdirSync3, writeFileSync as writeFileSync2 } from "node:fs";
import { dirname as dirname5, isAbsolute, join as join8, resolve as resolve4, sep } from "node:path";
...
function checkOutPath(dest, dataDir2) {
  const abs = isAbsolute(dest) ? resolve4(dest) : resolve4(process.cwd(), dest);
  if (existsSync6(abs)) {
    return { ok: false, abs, error: `${abs} already exists; refusing to overwrite it` };
  }
  const root = resolve4(dataDir2 || "");
...
`);
  ...
  const res = args.mode === "export" ? await exportActivity(cfg, shared2) : args.mode === "scan" ? await scanActivity(cfg, {
    ...shared2,
...
var USAGE, VALUED, BARE, selfPath, selfReal, entryPath;
var init_activity_src = __esm({
  async "../claude-code/bin/activity.src.mjs"() {
    init_activity();
    init_config();
    init_runid();
    init_state();
    USAGE = `mubit-memory: what this instance holds, and a copy of it you can keep.
...
Output
  ...
  -h, --help
`;
    VALUED = /* @__PURE__ */ new Set(["--run", "--type", "--since", "--until", "--agent", "--user", "--limit", "--max", "--out"]);
...
// cli/activity.mjs
var shared = await init_activity_src().then(() => activity_src_exports);
