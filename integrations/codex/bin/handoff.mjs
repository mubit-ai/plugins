...
async function postHandoff(cfg, req, opts = {}) {
  const started = Date.now();
  const bad = firstOf(
...
async function postFeedback(cfg, req, opts = {}) {
  const started = Date.now();
  const bad = firstOf(
...
// ../claude-code/lib/handoff.mjs
async function sendHandoff(cfg, o) {
  const action = str4(o?.requested_action).toLowerCase() || "continue";
  const res = await postHandoff(cfg, {
    run_id: str4(o?.run_id),
    task_id: str4(o?.task_id),
    from_agent_id: str4(o?.from_agent_id),
    to_agent_id: str4(o?.to_agent_id),
  ...
  return { ok: true, id: str4(res.body?.handoff_id), action };
}
async function submitFeedback(cfg, o) {
  const verdict = str4(o?.verdict).toLowerCase();
  const res = await postFeedback(cfg, {
    run_id: str4(o?.run_id),
    handoff_id: str4(o?.handoff_id),
    verdict,
    comments: typeof o?.comments === "string" ? o.comments : "",
    from_agent_id: str4(o?.from_agent_id)
  }, { record: false, timeoutMs: WRITE_TIMEOUT_MS });
  if (!res.ok) return failure(cfg, res);
  return { ok: true, id: str4(res.body?.feedback_id), verdict };
}
async function listHandoffs(cfg, runId, o = {}) {
  const run = str4(runId);
  ...
  const res = await listActivity(cfg, {
    ...
    projection: "full",
    limit: posInt2(o?.limit, LIST_LIMIT),
    sort: "desc"
  }, { record: false, timeoutMs: posInt2(o?.timeoutMs, LIST_TIMEOUT_MS) });
  if (!res.ok) return failure(cfg, res);
  ...
  for (const e of entries) {
    ...
    const type = str4(e.entry_type) || str4(meta.entry_type);
    const id = str4(e.id);
    if (!id) continue;
    const at = str4(e.created_at) || str4(meta.created_at);
    const from = str4(meta.from_agent_id) || sourceAgent(e.source);
    if (type === "handoff") {
      if (byId.has(id)) continue;
      byId.set(id, {
        id,
        task_id: str4(meta.task_id),
        from,
        to: str4(meta.to_agent_id),
        action: str4(meta.requested_action).toLowerCase(),
      ...
      answers.push({
        id,
        handoff_id: str4(meta.handoff_id),
        verdict: str4(meta.verdict).toLowerCase(),
...
function failure(cfg, res) {
  return {
    ...
    error: scrubKey2(cfg, str4(res?.error) || "the instance did not answer")
  };
}
function scrubKey2(cfg, text) {
  try {
    const s = String(text ?? "");
    const key = cfg && typeof cfg.apiKey === "string" ? cfg.apiKey.trim() : "";
    if (!key || !s.includes(key)) return s;
...
var LIST_TIMEOUT_MS, WRITE_TIMEOUT_MS, LIST_LIMIT, PREVIEW_CHARS;
var init_handoff = __esm({
  "../claude-code/lib/handoff.mjs"() {
    init_activity();
    init_http();
    LIST_TIMEOUT_MS = 15e3;
    WRITE_TIMEOUT_MS = 8e3;
    LIST_LIMIT = 500;
    PREVIEW_CHARS = 160;
  }
});
...
function scanRunMarkers(root) {
  const out = [];
  try {
    const dir = join7(str5(root), "status");
    if (!str5(root) || !existsSync5(dir)) return out;
...
var AGENT_ROLES, DEFAULT_AGENT_ROLE, AGENT_SHORT, HASH_LEN, TOUCH_INTERVAL_MS;
...
function pickRun(cfg, explicit = "", opts = {}) {
  const command = str6(opts?.command) || "pin";
  const named = str6(explicit) || (str6(cfg?.runStrategy) === "static" ? str6(cfg?.runId) : "");
  if (named) {
  ...
  return {
    ok: false,
    state: "no_run",
    detail: `Could not tell which Mubit run this session is using \u2014 no run marker in ${str6(cfg?.dataDir) || "(no data directory resolved)"}. If the session has been sending prompts, that is not the directory its hooks are writing to: /mubit-memory:doctor prints the one they use, and MUBIT_CC_DATA_DIR overrides it. Otherwise send one prompt first, or name the run: ${command} --run <run_id> \u2026.`
  };
}
function newestMarker(cfg) {
  const now = Date.now();
  const fresh2 = scanRunMarkers(str6(cfg?.dataDir)).filter((m) => m.runId !== POISONED_RUN_ID2 && m.at > 0 && now - m.at < MARKER_MAX_AGE_MS).sort((a, b) => b.at - a.at);
...
function markerBase(runId) {
  return str6(runId).replace(/-sub-[^-]+$/, "").replace(/-c\d+$/, "");
}
function str6(v) {
  return typeof v === "string" ? v.trim() : "";
}
...
// ../claude-code/bin/handoff.src.mjs
var handoff_src_exports = {};
__export(handoff_src_exports, {
  main: () => main,
  parseArgs: () => parseArgs
});
...
function parseArgs(argv = []) {
  ...
  const takesValue = /* @__PURE__ */ new Set(["--to", "--action", "--task", "--from", "--run", "--data-dir", "--verdict", "--comments"]);
  const known = /* @__PURE__ */ new Set([...takesValue, "--open", "--json", "--help", "-h"]);
  const positional = [];
  ...
  const verbs = /* @__PURE__ */ new Set(["send", "list", "ls", "feedback", "answer"]);
  const action = verbs.has(first) ? { ls: "list", answer: "feedback" }[first] ?? first : "list";
...
async function main(argv = process.argv.slice(2), env = process.env, deps = {}) {
  ...
  try {
    ...
    return emit({ ok: false, state: "config_error", detail: `Could not read the plugin configuration: ${messageOf2(err)}` });
  }
  if (!str7(cfg.endpoint)) {
    return emit({
      ok: false,
      state: "unconfigured",
...
function failed(what, runId, res) {
  return {
    ok: false,
    state: str7(res?.state) || "upstream_failed",
    run_id: runId,
    // `res.error` has already been scrubbed of the API key by `lib/http.mjs`.
    detail: `Could not ${what} ${runId}: ${str7(res?.error) || "the instance did not answer"}`
  };
}
...
var USAGE, selfPath, selfReal, entryPath;
var init_handoff_src = __esm({
  async "../claude-code/bin/handoff.src.mjs"() {
    init_config();
    init_handoff();
    init_runid();
    init_runpick();
    init_state();
    USAGE = `mubit-memory: hand work to another agent, list what is open, or answer a handoff.
...
// cli/handoff.mjs
var shared = await init_handoff_src().then(() => handoff_src_exports);
