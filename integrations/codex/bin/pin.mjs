...
function pickRun(cfg, explicit = "") {
  ...
  return {
    ok: false,
    state: "no_run",
    // Naming the directory matters more than it looks: the usual cause is not a session that
    // has never run a hook, but a command reading a different data directory from the one the
    // hooks write to. "Send one prompt first" is useless advice to someone who already has.
    detail: `Could not tell which Mubit run this session is using \u2014 no recent run marker in ${resolveDataDir(cfg)}. If the hooks are running, they may be writing somewhere else: check \`ls ~/.claude/plugins/data/\` and pass --data-dir=<path>. Otherwise send one prompt first, or name the run: pin --run <run_id> "\u2026". /mubit-memory:doctor prints the current run id.`
  };
}
