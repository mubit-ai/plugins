...
Call `mcp__mubit__mubit_learned`. Pick a template, which sets `lesson_type` and
`lesson_scope` for you:
...
low-boilerplate path, and every entry it writes is stored as `success` at `run` scope. The
plugin clamps it there on purpose — anything wider is read back by unrelated projects — and
`run` is not the memory loss it sounds like. The default `runStrategy` is `per-directory`, so
