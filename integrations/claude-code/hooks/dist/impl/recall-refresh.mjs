...
import { basename as basename2, dirname as dirname3, join as join10, resolve as resolve2 } from "node:path";
  ...
  body: async (payload, _hookCfg, ctx) => {
    ...
    if (outcome.failed) {
      log(
        cfg,
        "warn",
        `recall-refresh: recall failed on rung ${outcome.rung} (${str4(outcome.state) || "unknown"})`,
        { run_id: runId, error: str4(outcome.error).slice(0, 300) }
      );
      updateMarker(cfg, runId, {
        ...isConnState2(str4(outcome.state)) ? { state: str4(outcome.state) } : {},
