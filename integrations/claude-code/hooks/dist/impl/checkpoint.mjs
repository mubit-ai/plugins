...
import { closeSync as closeSync3, openSync as openSync3, readSync, statSync as statSync6 } from "node:fs";
...
function breakerPath(cfg = {}) {
  return join7(resolveDataDir(cfg), "breaker", `${endpointHash(cfg)}.json`);
}
...
function resumePath(cfg, runId) {
  if (!safeSegment(runId)) return "";
  return join9(runDir(cfg, runId), "resume.json");
}
...
function checkpointsPath(cfg, runId) {
  return join12(resolveDataDir(cfg), "runs", safeSegment(runId), "checkpoints.json");
}
