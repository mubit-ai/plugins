...
import { existsSync as __mubitExists, readFileSync as __mubitRead, readdirSync as __mubitReaddir, statSync as __mubitStat } from "node:fs";
 ...
 *
 * Hooks and the MCP server are launched with MUBIT_CC_DATA_DIR pinned into them by setup, but a
 * command a model runs straight from the shell inherits none of that \u2014 and the directory's
 * suffix varies with the install (`mubit-memory-<marketplace>`, `mubit-memory-inline`, ...).
 * Returning the bare name on that path sent these commands to scan an empty directory and
 * report "no hook has written a run marker yet" at a session whose hooks had been writing
 * markers all along.
 * @param {string} home
...
function liveDataDir(home, env = {}) {
  ...
  // What setup pinned into the live Codex registrations is the recorded answer, not a guess,
  // so it outranks any search: it is the directory the hooks themselves are using.
  try {
    const codexHome = typeof env.CODEX_HOME === "string" && env.CODEX_HOME ? env.CODEX_HOME : join(home, ".codex");
    const pinned = JSON.stringify(JSON.parse(__mubitRead(join(codexHome, "hooks.json"), "utf8"))).match(
      /MUBIT_CC_DATA_DIR=\\"([^\\"]+)\\"/
    );
  ...
  try {
    let best = "";
    let bestAt = -1;
    for (const name of __mubitReaddir(root)) {
      if (!name.startsWith("mubit-memory")) continue;
      const dir = join(root, name);
      let at = 0;
      try {
        for (const f of __mubitReaddir(join(dir, "status"))) {
          if (!f.endsWith(".json") || f === "health.json") continue;
          at = Math.max(at, __mubitStat(join(dir, "status", f)).mtimeMs);
        }
      } catch {
      }
      // A directory holding credentials outranks a livelier one that holds none: that is the
      // install which was actually signed in, and the liveliness of the rest is noise.
      if (__mubitExists(join(dir, "credentials.json"))) at += 1e15;
      if (at > bestAt) {
        bestAt = at;
        best = dir;
      }
