...
function relativeToRepo(p, projectDir, root) {
  ...
  if (isAbsolute(rel)) {
    ...
    for (const base of /* @__PURE__ */ new Set([resolve3(projectDir || root), root])) {
      if (abs === base) return "";
...
function ledgerPath(dir, runId) {
  return join8(runDir({ dataDir: dir }, safeSegment(runId) || "unknown"), LEDGER_FILE);
}
...
function appendLedger(dir, runId, row) {
  try {
    ...
    if (!ensureDir(dirname6(p))) return false;
...
var FILES_FILE = "files.json";
var VERSION = 1;
var MAX_CHANGES_PER_EVENT = 32;
var MAX_INDEXED_PATHS = 256;
...
import { existsSync as existsSync6, readdirSync as readdirSync2, readFileSync as readFileSync6, statSync as statSync5 } from "node:fs";
import { basename as basename2, dirname as dirname7, join as join10, resolve as resolve5 } from "node:path";
...
function hasDeniedSubject(payload, cfg) {
  ...
  const projectDir = str5(cfg?.projectDir);
