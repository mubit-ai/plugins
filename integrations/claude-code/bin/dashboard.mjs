...
var FEED_PREFIX = /^state::[^:]*::/;
var SUB_SUFFIX = /-sub-[a-z0-9]+$/;
var CLEAR_SUFFIX = /-c(\d+)$/;
var DIRECTORY_KEY = /^cc-.+-[0-9a-f]{8}$/;
...
function runsIn(dir, opts = {}) {
  ...
  return lsDir(join9(dir, "status")).filter(isRunMarker).map((f) => {
    ...
    const row = {
      ...
      lastWrite: num3(marker.updated_at) || mtimeOf(join9(dir, "status", f)),
    ...
    if (byRun) {
      ...
      Object.assign(row, {
        ...
        subagentCount: lsDir(join9(rd, "subagents")).filter((n) => n.endsWith(".json")).length
      });
    }
    return row;
...
function familyOf(dir, runId) {
  ...
  for (const name of lsDir(join9(dir, "runs"))) {
    if (known.has(name) || safeSegment(name) !== name) continue;
    const ledger = join9(dir, "runs", name, LEDGER_FILE);
    if (existsSync5(ledger)) known.set(name, { runId: name, lastWrite: mtimeOf(ledger) });
  }
...
function readSubagents(dir, runId, opts = {}) {
  const id = safeSegment(runId);
  if (!id) return [];
  const sdir = join9(runDir({ dataDir: dir }, id), "subagents");
  const since = num3(opts && opts.since);
  const out = [];
  for (const f of lsDir(sdir)) {
    if (!f.endsWith(".json")) continue;
    const p = join9(sdir, f);
...
var OUTCOME_WORDS2 = /* @__PURE__ */ new Set(["success", "failure", "neutral", "none"]);
...
function turnDetail(dir, runId, promptId) {
  ...
  const p = join9(runDir({ dataDir: dir }, run), "turns", `${id}.json`);
...
function verdictsPath(dir, runId) {
  return join9(dir, ROLLUP_DIR, `verdicts-${safeSegment(runId) || "unknown"}.jsonl`);
}
...
var COOKIE_NAME = "mubit_dashboard";
...
var NO_INJECTIONS = Object.freeze({
  injectedCount: 0,
  usedInTurns: 0,
  lastInjectedAt: 0,
