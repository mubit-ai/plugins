...
var INJECTED_USER_RE = /^\s*(?:<(?:environment_context|recommended_plugins|turn_aborted|user_shell_command|skill|image)\b|# AGENTS\.md instructions|# Files mentioned)/;
...
import { existsSync as existsSync6, readdirSync as readdirSync3, statSync as statSync5 } from "node:fs";
...
// lib/filechange.mjs
var FILE_CHANGE_KINDS = Object.freeze(["add", "delete", "update"]);
var MAX_CHANGES_PER_EVENT = 32;
var MAX_PATH_CHARS = 512;
var PATH_KEYS = ["file_path", "filePath", "path", "notebook_path", "notebookPath", "target_file"];
var PRIOR_KEYS = ["old_string", "oldString", "old_source", "oldSource", "old_str"];
var CONTENT_KEYS = ["content", "new_string", "newString", "new_source", "newSource", "new_str"];
var PATCH_BODY_KEYS = ["command", "patch", "input"];
var PATCH_SENTINEL = "*** Begin Patch";
var PATCH_FILE_RE = /^\*\*\* (Add|Delete|Update) File:[ \t]*(.*)$/gm;
var PATCH_MOVE_RE = /^\*\*\* Move to:[ \t]*(.*)$/gm;
...
import { dirname as dirname3, isAbsolute, join as join5, resolve as resolve2, sep } from "node:path";
...
function warmIgnoreCache(paths, projectDir) {
  try {
    const root = gitRootOf(projectDir);
    if (!root) return 0;
...
function relativeToRepo(p, projectDir, root) {
  ...
  if (isAbsolute(rel)) {
    ...
    for (const base of /* @__PURE__ */ new Set([resolve2(projectDir || root), root])) {
      if (abs === base) return "";
...
var PATH_KEYS2 = ["file_path", "filePath", "path", "notebook_path", "notebookPath", "target_file"];
...
// lib/import.mjs
var HOST = "claude-code";
var TRANSCRIPT_ROOT_ENV = "MUBIT_CC_TRANSCRIPT_ROOT";
var CURSOR_DIR = "import";
var CURSOR_VERSION = 1;
var PERSISTED_MARK = "<persisted-output>";
var HOST_NOTICE_RE = /^(?:<(?:local-command-caveat|command-name|command-message|command-args|system-reminder|persisted-output)\b|\[Request interrupted|\/)/;
var DEFAULT_BATCH = 32;
var DEFAULT_PACE_MS = 200;
var DEFAULT_TIMEOUT_MS2 = 3e4;
var DEFAULT_MAX_ITEMS = 5e3;
var MAX_TAIL_CHARS = 4e3;
var MAX_TEXT_CHARS = 8e3;
var MAX_RENDER_ITEMS = 24;
var MAX_VALUE_CHARS = 400;
var MAX_RENDER_DEPTH = 2;
...
function transcriptRoot(env = process.env) {
  ...
  return override || join8(homedir2(), ".claude", "projects");
}
function discoverTranscripts(opts = {}) {
  ...
  try {
    for (const dir of dirEntries(root)) {
      ...
      dirs += 1;
      const dirPath = join8(root, dir.name);
      for (const e of dirEntries(dirPath)) {
        if (e.isFile() && e.name.endsWith(".jsonl")) {
          files.push({
            path: join8(dirPath, e.name),
...
function linkedRoots(projectDir) {
  const start = typeof projectDir === "string" ? projectDir.trim() : "";
  if (!start) return [];
  const out = [resolve4(start)];
  try {
    ...
    for (const line of r.stdout.split("\n")) {
      ...
      const p = resolve4(line.slice("worktree ".length).trim());
...
function rootFor(cwd, roots) {
  const c = typeof cwd === "string" && cwd.trim() ? resolve4(cwd.trim()) : "";
  if (!c) return "";
  for (const r of Array.isArray(roots) ? roots : []) {
    const base = resolve4(String(r));
...
function cursorPath(cfg, path) {
  ...
  return join8(resolveDataDir(cfg), CURSOR_DIR, `${h}.json`);
}
...
function writeCursor(cfg, path, cur) {
  try {
    ...
    if (!ensureDir(join8(resolveDataDir(cfg), CURSOR_DIR))) return false;
...
function buildToolItem(cfg, call, result, opts = {}) {
  try {
    const projectDir = call.root || call.cwd || str3(opts?.projectDir) || str3(cfg?.projectDir);
...
function deniedSubject(call, cfg, projectDir) {
  ...
  for (const c of Array.isArray(call.files) ? call.files : []) {
    if (isObject4(c) && typeof c.path === "string" && c.path && isDeniedPath(c.path, cfg, projectDir)) return true;
  }
  return false;
}
var PATH_KEYS3 = ["file_path", "filePath", "path", "notebook_path", "notebookPath", "target_file"];
...
function buildTurnItem(cfg, turn, opts = {}) {
  try {
    ...
    const projectDir = turn.root || turn.cwd || str3(opts?.projectDir) || str3(cfg?.projectDir);
    const runId = runIdFor(cfg, projectDir);
    if (!runId) return null;
    const q = attempt(
...
var _runIdCache = /* @__PURE__ */ new Map();
function runIdFor(cfg, projectDir) {
  const dir = str3(projectDir);
  if (!dir) return "";
...
var claudeCodeSource = Object.freeze({
  name: "claude-code",
  host: HOST,
...
async function runImport(cfg, opts = {}) {
  ...
  const roots = opts?.all === true ? [] : Array.isArray(opts?.roots) && opts.roots.length ? opts.roots.map((r) => resolve4(String(r))) : linkedRoots(str3(cfg?.projectDir) || process.cwd());
...
function dirEntries(p) {
  try {
    return existsSync6(p) && statSync5(p).isDirectory() ? readdirSync3(p, { withFileTypes: true }) : [];
  } catch {
    return [];
  }
}
...
// lib/codex-import.mjs
var CODEX_SESSIONS_ROOT_ENV = "MUBIT_CC_CODEX_SESSIONS_ROOT";
var HOST2 = "codex";
var ITEMS_SINCE = [0, 149, 0];
var TOOL_ITEM_TYPES = /* @__PURE__ */ new Set(["CommandExecution", "FileChange", "McpToolCall"]);
var SHELL_CALLS = /* @__PURE__ */ new Set(["exec_command", "shell", "local_shell", "container.exec"]);
var SHELL_ARG_KEYS = ["cmd", "command"];
var MAX_TEXT_CHARS2 = 8e3;
var DATE_DEPTH = 3;
var CHANGE_KINDS = /* @__PURE__ */ new Set(["add", "delete", "update"]);
function rolloutRoot(env = process.env) {
  const override = str4(env?.[CODEX_SESSIONS_ROOT_ENV]);
  if (override) return override;
  const home = str4(env?.CODEX_HOME);
  return home ? join9(home, "sessions") : join9(homedir3(), ".codex", "sessions");
}
  ...
  try {
    ...
    while (stack.length) {
      ...
      for (const e of dirEntries2(dir)) {
        if (e.isDirectory()) {
          if (depth < DATE_DEPTH) stack.push([join9(dir, e.name), depth + 1]);
          continue;
        }
        if (!e.isFile() || !e.name.startsWith("rollout-") || !e.name.endsWith(".jsonl")) continue;
        files.push({
          path: join9(dir, e.name),
          sessionId: "",
          agentId: "",
...
function rolloutItems(cfg, path, opts = {}) {
  ...
  const build = (call, result) => {
    const built = buildToolItem(cfg, { ...call, host: HOST2 }, result, opts);
    if (built === "denied") {
      denied += 1;
      return;
    }
    ...
    if (type === "response_item") {
      ...
      if (pt === "message") {
        ...
        if (role === "user") {
          ...
          const done = buildTurnItem(cfg, turn, opts);
          if (done) items.push(done);
          turn = { prompt: text, answer: [], ...where, host: HOST2 };
...
// bin/import.src.mjs
var USAGE = `mubit-memory: backfill memory from the transcripts already on this machine.
...
Output
  ...
  -h, --help
`;
var VALUED = /* @__PURE__ */ new Set(["--project", "--source", "--max", "--max-files", "--batch", "--pace"]);
var BARE = /* @__PURE__ */ new Set(["--all", "--dry-run", "--send", "--json", "--help", "-h"]);
var SOURCES2 = Object.freeze({ "claude-code": claudeCodeSource, codex: codexSource });
...
${USAGE}`);
  ...
  const projectDir = args.project ? resolve5(args.project) : cfg.projectDir || process.cwd();
